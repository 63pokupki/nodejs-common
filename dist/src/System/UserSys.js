...
class UserSys {
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let ifAuth = yield this.userSQL.isAuth(this.apikey);
            if (ifAuth) {
                this.req.sys.bAuth = true;
            }
            let userInfoList = {};
            if (ok && ifAuth) {
                userInfoList = yield this.userSQL.fGetUserInfoByApiKey(this.apikey);
                if (!userInfoList) {
                    ok = false;
            ...
            let userGroupsList = {};
            if (ok && ifAuth) {
                userGroupsList = yield this.userGroupSQL.getUserGroupsByUserID(this.idUser);
                if (!userGroupsList) {
                    ok = false;
            ...
            if (ok) {
                ctrlAccessListTemp = yield this.ctrlAccessSQL.getAllCtrlAccess();
                if (!userGroupsList) {
                    ok = false;
    ...
    isAccessCtrl(alias) {
        return __awaiter(this, void 0, Promise, function* () {
            ...
            if (ok) {
                ifCtrlAccess = yield this.accessGroupSQL.getAccess(idsGroupList, this.idCtrlAccess);
                if (!ifCtrlAccess) {
                    ok = false;
            ...
            let accessCRUDList = [];
            if (ok) {
                accessCRUDList = yield this.accessGroupSQL.getAccessCRUD(idsGroupList, this.idCtrlAccess);
                if (!accessCRUDList) {
                    ok = false;
    ...
    isAuth() {
        return __awaiter(this, void 0, Promise, function* () {
            ...
            if (ok && (yield this.userSQL.isAuth(this.apikey))) {
